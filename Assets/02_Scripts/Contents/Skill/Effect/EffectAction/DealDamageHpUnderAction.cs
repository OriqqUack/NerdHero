...
[System.Serializable]
public class DealDamageHpUnderAction : EffectAction
{
    [SerializeField]
    ...
    [SerializeField]
    private float hpUnderRate;
    [SerializeField]
    private float hpUnderStatFactor;
    ...
    public override bool Apply(Effect effect, Entity user, Entity target, int level, int stack, float scale)
    {
        ...
        float rate = target.Stats.HPStat.Value / target.Stats.MaxHpStat.Value;
        if (rate <= hpUnderRate)
            totalDamage *= (1 + hpUnderStatFactor);
    ...
    {
        return new DealDamageHpUnderAction()
        {
            ...
            hpUnderRate = hpUnderRate,
            hpUnderStatFactor = hpUnderStatFactor
        };
    }
}
