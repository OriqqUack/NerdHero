...
[System.Serializable]
public class DealPerDamageAction : EffectAction
{
    [SerializeField] 
    ...
    private float GetDefaultDamage(Effect effect)
        => defaultPerDamage + (effect.DataBonusLevel * bonusDamagePerLevel);
    ...
    public override bool Apply(Effect effect, Entity user, Entity target, int level, int stack, float scale)
    {
        ...
        var damage = GetTotalDamage(effect, user, stack, scale);
        float totalDamage = target.Stats.HPStat.MaxValue * damage;
    ...
    {
        return new DealPerDamageAction()
        {
