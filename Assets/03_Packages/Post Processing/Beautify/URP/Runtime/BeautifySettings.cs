...
namespace Beautify.Universal {
    ...
    public class BeautifySettings : MonoBehaviour {
        ...
        public void BlinkDotween(float duration, float maxValue = 1)
        {
            if (duration <= 0)
                return;
            BeautifySettings i = instance;
            ...
            beautify.vignettingInnerRing.overrideState = true;

            DOTween.To(() => beautify.vignettingBlink.value, x => beautify.vignettingBlink.value = x, maxValue, duration)
                .SetEase(blinkAnimationCurve)
                .OnComplete(() => VignettingOff()
                );
        }
        ...
        public void BlurFade(float duration, float maxValue = 1)
        {
            if (duration <= 0)
                return;
            BeautifySettings i = instance;
            ...
            DOTween.To(() => beautify.blurIntensity.value, x => beautify.blurIntensity.value = x, maxValue, duration)
                .OnComplete(() => beautify.blurIntensity.overrideState = false);
        }

        public void TakeDamage(float duration, float maxValue = 1)
        {
            ...
            beautify.vignettingInnerRing.overrideState = true;
            beautify.vignettingInnerRing.value = 0f; // 강제 초기화

            DOTween.To(() => beautify.vignettingInnerRing.value, 
                    x => beautify.vignettingInnerRing.value = x, 
                    maxValue, duration)
        ...
        private void VignettingOff()
        {
            BeautifySettings i = instance;
            if (i == null) return;
            Beautify beautify = settings;
            
            beautify.vignettingColor = new ColorParameter(Color.black);
